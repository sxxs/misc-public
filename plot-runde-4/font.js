...
const PR4Font = (() => {
    const G = {
        ...
        "*": { w: 4.4, s: [[[2.2, 7.6], [2.2, 2.4]], [[0.3, 6.4], [4.1, 3.6]],
            [[0.3, 3.6], [4.1, 6.4]]] },
        " ": { w: 4, s: [] },
        "(": { w: 2.6, s: [[[2.4, 10.5], [0.9, 8], [0.4, 5], [0.9, 2], [2.4, -0.5]]] },
        ")": { w: 2.6, s: [[[0.2, 10.5], [1.7, 8], [2.2, 5], [1.7, 2], [0.2, -0.5]]] },
        "=": { w: 4.8, s: [[[0.3, 6.1], [4.5, 6.1]], [[0.3, 3.7], [4.5, 3.7]]] },
        "^": { w: 3.2, s: [[[0.3, 8.4], [1.6, 10], [2.9, 8.4]]] },
        "/": { w: 4, s: [[[0.2, -0.5], [3.8, 10.5]]] },
        "'": { w: 1.6, s: [[[0.8, 10], [0.8, 8.2]]] },
        ",": { w: 1.8, s: [[[1, 0.6], [0.6, -1]]] },
        "π": { w: 6.4, s: [[[0, 7.4], [0.9, 8.1], [6.4, 8.1]],
            [[1.7, 8.1], [1.5, 0]], [[4.7, 8.1], [4.6, 1], [5, 0.1], [5.8, 0.4]]] },
        "Σ": { w: 5.6, s: [[[5.6, 10], [0, 10], [3.4, 5], [0, 0], [5.6, 0]]] },
        "∞": { w: 7.6, s: [[[3.8, 5], [2.8, 6.6], [1.4, 6.9], [0.3, 5.9], [0.3, 4.1],
            [1.4, 3.1], [2.8, 3.4], [3.8, 5], [4.8, 6.6], [6.2, 6.9], [7.3, 5.9],
            [7.3, 4.1], [6.2, 3.1], [4.8, 3.4], [3.8, 5]]] },
        "∫": { w: 3.8, s: [[[3.6, 9.7], [2.9, 10.4], [2.1, 9.8], [1.9, 8.5],
            [1.9, 1.5], [1.7, 0.2], [0.9, -0.4], [0.2, 0.3]]] },
        "λ": { w: 5.4, s: [[[0.5, 10], [1.7, 9.6], [5.2, 0]], [[3.2, 5.6], [0, 0]]] },
        "Ω": { w: 6.6, s: [[[0.2, 0], [2.3, 0], [1.3, 2], [0.7, 4], [0.9, 6.8],
            [2.3, 9.3], [4.3, 9.3], [5.7, 6.8], [5.9, 4], [5.3, 2], [4.3, 0], [6.4, 0]]] },
        "√": { w: 6.8, s: [[[0, 4.6], [1.6, 4.6], [2.9, 0], [4.5, 10], [6.8, 10]]] },
        "φ": { w: 6.2, s: [[[3.1, 7.9], [4.9, 7], [5.7, 5], [4.9, 3], [3.1, 2.1],
            [1.3, 3], [0.5, 5], [1.3, 7], [3.1, 7.9]], [[3.1, 10], [3.1, 0]]] },
        "·": { w: 2, s: [[[0.8, 4.6], [1.2, 4.6], [1.2, 5.1], [0.8, 5.1], [0.8, 4.6]]] }
    };
    const MAP = { "Ä": "A", "Ö": "O", "Ü": "U", "ß": "S" };

    function normalize(text) {
        return String(text || "").split("").map(c => {
            if (G[c]) return c;                       // direct hit (math symbols etc.)
            const u = c.toUpperCase();
            if (G[u]) return u;
            return MAP[u] || MAP[c] || "";
        }).filter(c => c && G[c]).join("") || "A";
    }
